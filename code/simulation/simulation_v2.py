# directional_stereo_beamforming_angles_fixed_music.py
import numpy as np
...
# -----------------------------
# 3. Define speaker array (angled around listeners)
# -----------------------------
num_speakers = 120
theta_range = np.linspace(-np.pi/4, np.pi/4, num_speakers)  # ±45° span
radius = 2.0  # radius of arc
mid_listener_x = 2.5  # centre of arc
z_height = 1.0

speakers = np.zeros((3, num_speakers))
for i, theta in enumerate(theta_range):
    x = np.clip(mid_listener_x + radius * np.sin(theta), 0, room_dim[0])
    y = room_dim[1]  # top wall
    speakers[:, i] = [x, y, z_height]

# -----------------------------
# 4. Define listener positions (closer together)
# -----------------------------
listener_a_pos = np.array([3.25, 1.0, 1.0])
listener_b_pos = np.array([1.75, 1.0, 1.0])
...
# -----------------------------
stereo_a_angles = apply_beamforming(listener_a_pos, speakers, speaker_signals, sr_a)
stereo_b_angles = apply_beamforming(listener_b_pos, speakers, speaker_signals, sr_a)

# Reduce amplitude to avoid clipping
stereo_a_angles *= 0.25
stereo_b_angles *= 0.25

# Save audio files with angle identifier
sf.write("audio/a_angles120.wav", stereo_a_angles, sr_a)
sf.write("audio/b_angles120.wav", stereo_b_angles, sr_a)
print("Stereo angled audio files saved for listeners A and B.")
...
plt.scatter(speakers[0, :], speakers[1, :], c='black', marker='o', label='Speakers')  # <-- add this
